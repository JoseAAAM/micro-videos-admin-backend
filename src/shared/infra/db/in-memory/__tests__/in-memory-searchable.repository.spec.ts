...
describe('InMemorySearchableRepository Unit Tests', () => {
  ...
  describe('applyFilter method', () => {
    it("should not filter items when filter param is null", async () => {
      ...
      const spyFilterMethods = jest.spyOn(items, 'filter' as any)
      const itemsFiltered = await repository['applyFilter'](items, null)
      expect(itemsFiltered).toStrictEqual(items)
      expect(spyFilterMethods).not.toHaveBeenCalled()
    })
    ...
    it('should filter using a filter param', async () => {
      ...
      expect(spyFilterMethods).toHaveBeenCalledTimes(1)

      itemsFiltered = await repository['applyFilter'](items, '5')
      expect(itemsFiltered).toStrictEqual([items[0], items[1]])
      expect(spyFilterMethods).toHaveBeenCalledTimes(2)

      itemsFiltered = await repository['applyFilter'](items, 'no-filter')
      expect(itemsFiltered).toHaveLength(0)
      expect(spyFilterMethods).toHaveBeenCalledTimes(3)
    })
  })
